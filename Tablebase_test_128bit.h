...
class Tablebase_test {
    ...
    */
     ...
     * @return
     */
    boost::multiprecision::uint128_t index(std::vector<int> &pieces) {
    ...
    void generate_and_store_all_positions(std::vector<int> &pieces_so_far, std::size_t total_piece, std::vector<uint8_t> &tablebase,
        ...
        if (piece_count_so_far == total_piece) {
            boost::multiprecision::uint128_t id = index(pieces_so_far);
            auto small = (uint64_t) id;
            tablebase[small]++;
            return;
        }
...
public:
    ...
    void test_indexing(std::size_t num_pieces) {
        boost::multiprecision::int128_t size = n_choose_k(164, num_pieces);
        std::vector<uint8_t> tablebase((uint64_t) size);
